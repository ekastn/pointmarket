...
func (s *RecommendationService) EnrichItemRefsWithTitles(ctx context.Context, payload map[string]interface{}) map[string]interface{} {
	...
	for _, it := range rawItems {
		...
		switch rtype {
		case "mission":
			if r, ok := mByID[rid]; ok {
				title = r.Title
			}
		case "reward":
			if r, ok := rByID[rid]; ok {
				title = r.Title
			}
		case "punishment":
			if r, ok := pByID[rid]; ok {
				title = r.Title
			}
		case "coaching":
			if r, ok := cByID[rid]; ok {
				title = r.Title
			}
		case "product":
			if r, ok := prByID[rid]; ok {
				title = r.Name
			}
		}
		if title != "" {
...
// Admin States proxies
func (s *RecommendationService) AdminStatesList(ctx context.Context, q url.Values) (map[string]interface{}, error) {
	return s.gateway.AdminStatesList(ctx, q)
}

func (s *RecommendationService) AdminStatesCreate(ctx context.Context, body []byte) (map[string]interface{}, error) {
	return s.gateway.AdminStatesCreate(ctx, body)
}

func (s *RecommendationService) AdminStatesUpdate(ctx context.Context, id int64, body []byte) (map[string]interface{}, error) {
	return s.gateway.AdminStatesUpdate(ctx, id, body)
}
