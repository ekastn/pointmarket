...
class AdminItemsController extends BaseController
{
    ...
    public function index(): void
    {
        ...
        $data = $this->itemsService->list($filters) ?? ['items' => [], 'meta' => ['total' => 0, 'limit' => 20, 'offset' => 0]];
    ...
    public function update(): void
    {
        ...
        if (isset($payload['action_code'])) $payload['action_code'] = (int)$payload['action_code'];
        if (isset($payload['ref_id'])) $payload['ref_id'] = (int)$payload['ref_id'];
        if (isset($payload['is_active'])) $payload['is_active'] = (bool)$payload['is_active'];
    ...
    public function destroy(): void
    {
        ...
        $resp = $this->itemsService->delete($id, $force);
        if (!empty($resp['success'])) {
    ...
    public function typeaheadRefs(): void
    {
        ...
        $q = trim($_GET['q'] ?? '');
        $limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 20;
        if ($t === '') { echo json_encode(['refs'=>[]]); return; }
