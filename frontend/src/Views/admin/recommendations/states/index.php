...
<div class="container-fluid">
  ...
  <?php elseif (!empty($messages['error'])): ?>
    <div class="alert alert-danger">
      <?= htmlspecialchars(is_array($messages['error']) ? json_encode($messages['error']) : $messages['error']) ?></div>
  <?php endif; ?>
          ...
          <form method="get" class="row g-2 align-items-end">
            <div class="col-md-4">
              ...
              <input type="text" name="q" class="form-control" value="<?= htmlspecialchars($filters['q'] ?? '') ?>"
                placeholder="State/Description contains..." />
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-2">
              ...
              <select name="limit" class="form-select">
                <?php foreach ([20, 50, 100] as $opt): ?>
                  <option value="<?= $opt ?>" <?= ((int) ($filters['limit'] ?? 20) === $opt) ? 'selected' : '' ?>><?= $opt ?>
                  </option>
                <?php endforeach; ?>
                <option value="all" <?= (($filters['limit']??'') === 'all' || ($filters['limit']??0) > 1000)?'selected':'' ?>>All</option>
              </select>
            </div>
            <div class="col-md-2">
    ...
    <div class="col-12">
      <div class="card shadow-sm">
        <div class="card-header d-flex align-items-center justify-content-between">
          ...
          <a href="/admin/recommendations/states/export" target="_blank" class="btn btn-sm btn-outline-success"><i
            ...
            <div class="row g-2 align-items-end">
              <div class="col-md-6">
                ...
                <input type="text" name="state" class="form-control"
                  placeholder="e.g., V_high_mslq_medium_ams_extrinsic_eng_low" required />
                <div class="form-text">Format:
                  [V|A|R|K]_high_mslq_[low|medium|high]_ams_[amotivation|extrinsic|achievement|intrinsic]_eng_[low|medium|high]
                </div>
              </div>
              <div class="col-md-4">
  ...
  $actions = [
    [
      'label' => 'Edit',
      'icon' => 'fas fa-edit',
      'class' => 'btn-outline-primary js-edit-state',
  ...
  <?php foreach ($states as $st):
    if (is_array($st)): ?>
      <form id="del-<?= (int) ($st['id'] ?? 0) ?>" method="post" action="/admin/recommendations/states/delete"
        style="display:none;">
